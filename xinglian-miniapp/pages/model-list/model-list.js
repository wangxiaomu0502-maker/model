...
Page({
  ...
  normalizeModelList(list) {
    ...
    return arr.map((item) => {
      ...
      const cardThumbUrls = angles.map((a) => this.resolveMediaUrl(a && a.url, apiBase)).filter(Boolean);
  ...
  loadModelList() {
    ...
    wx.request({
      ...
      success: (res) => {
        ...
        this.setData({ modelList: this.normalizeModelList(body.list), loadError: "" });
      },
      fail: () => {
