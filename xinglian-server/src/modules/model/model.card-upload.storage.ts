...
export async function uploadModelPortfolioImageToCos(input: {
  userId: number;
  body: Buffer;
  mimetype: string;
}): Promise<string> {
  if (!input.body || input.body.length === 0) {
    throw new AppError("empty portfolio image", 400, ErrorCodes.VALIDATION_ERROR);
  }

  const ext = extFromMime(input.mimetype);
  const objectKey = `models/portfolio/${input.userId}/${Date.now()}${ext}`;
  ...
  try {
    ...
    throw new AppError(
      `failed to upload portfolio image to cos (${getCosErrorMessage(err)})`,
      502,
...
export async function uploadModelCardImageToCos(input: {
  ...
  if (!input.body || input.body.length === 0) {
    throw new AppError("empty model card image", 400, ErrorCodes.VALIDATION_ERROR);
  }

  const ext = extFromMime(input.mimetype);
  const objectKey = `models/cards/${input.userId}/${Date.now()}${ext}`;
  ...
  try {
    ...
    throw new AppError(
      `failed to upload model card image to cos (${getCosErrorMessage(err)})`,
      502,
      ErrorCodes.UPSTREAM_ERROR
    );
  }
