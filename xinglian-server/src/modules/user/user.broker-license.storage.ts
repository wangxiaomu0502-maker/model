...
export async function uploadBrokerLicenseToCos(input: {
  userId: number;
  body: Buffer;
  mimetype: string;
}): Promise<string> {
  if (!input.body || input.body.length === 0) {
    throw new AppError("empty broker license image", 400, ErrorCodes.VALIDATION_ERROR);
  }

  const ext = extFromMime(input.mimetype);
  const objectKey = `brokers/license/${input.userId}/${Date.now()}${ext}`;
  ...
  try {
    ...
    throw new AppError(
      `failed to upload broker license to cos (${getCosErrorMessage(err)})`,
      502,
